...
for (const provider of ['claude', 'codex']) {
  const list = runAgent(provider, { home, cwd: join(home, 'work'), prompt: 'List the names of every skill available to you in this session, one per line, nothing else.' });
  const managed = provider === 'claude' ? ['probe claude root', 'MARK-CLAUDE-ROOT'] : ['probe agents root', 'MARK-AGENTS-ROOT'];
  const use = runAgent(provider, { home, cwd: join(home, 'work'), prompt: `Give me the ${managed[0]}.` });
  results[provider] = {
    lists: { claudeRoot: /probe-claude-root/.test(list.text), codexRoot: /probe-codex-root/.test(list.text), agentsRoot: /probe-agents-root/.test(list.text) },
...
const ok = results.claude.lists.claudeRoot && results.claude.usesManagedRoot && results.codex.lists.agentsRoot && results.codex.usesManagedRoot;
